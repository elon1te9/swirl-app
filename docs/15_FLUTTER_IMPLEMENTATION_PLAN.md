...
- `GET /api/avatars` is public in the current backend.
...
- Flutter must not expect exactly 4 avatars. Show whatever `GET /api/avatars` returns.
...
- Create `LoginScreen`.
- Create `SignUpScreen`.
- Load avatars with `GET /api/avatars`.
...
Use `FutureProvider` for simple read-only API screens.

Use `Notifier` or `AsyncNotifier` for flows with actions and local state.
