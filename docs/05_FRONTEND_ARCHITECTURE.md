...
- JSON response parsing into models

### domain

Contains simple app models.
...
Use Riverpod lightly.

Recommended approach:

- providers create shared dependencies such as Dio, API classes, storage, and controllers
- form input, loading flags, selected ids, and error text stay inside `StatefulWidget` screens when possible
- controllers call API/storage and return simple results
- avoid putting API calls directly inside widgets
- avoid large global mutable state
- do not add repositories, service interfaces, `Notifier`, `AsyncNotifier`, or `StateNotifier` unless the current simple approach becomes hard to read
...
- parse JSON into simple models
- handle Dio errors without crashing the app
