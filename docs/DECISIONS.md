...
During Stage 2 registration, Flutter assigns a random predefined avatar from
the simple MVP avatar set.
...
Use Riverpod lightly in Flutter.

For the student MVP, Riverpod should mostly create dependencies:
...
Keep form state, loading flags, selected ids, and error text inside
`StatefulWidget` screens when that is easier to read.

Do not add repositories, service interfaces, `Notifier`, `AsyncNotifier`, or
`StateNotifier` unless the simple screen/controller/API structure becomes hard
to maintain.
